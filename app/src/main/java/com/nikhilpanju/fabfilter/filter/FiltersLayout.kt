...
class FiltersLayout @JvmOverloads constructor(
    context: Context,
    ...
    private val sheetHeight: Float by lazy { screenWidth.toFloat() / 1.20f }
    ...
    private val viewPagerTranslateAmount = 0f.dp
    ...
    private fun openFilterSheet(isOpening: Boolean) {
        ...
        val helper1 = CircleCardViewAnimatorHelper(
            cardView = fab, startSize = fabSize,
            endSize = sheetWidth, startX = fabX2, startY = fabY2
        )
        val helper2 = CardViewAnimatorHelper(
            cardView = fab,
        ...
        val settleAnimator = getValueAnimator(
            isOpening, settleAnimDuration,
            ...
            bottomBarCardView.setCardBackgroundColor(
                blendColors(
                    bgColor,
                    if (tabsHandler.hasActiveFilters) bottomBarPinkColor else bottomBarColor,
                    progress
                )
            )
    ...
    private fun unFilterAdapterWithFabAnimation() {
        ...
        val insetProp = CircleCardViewAnimatorHelper(
            fab,
